...
const defaultResultsTemplate = (props, state, styles) => {
...
export default class FuzzySearch extends Component {
  ...
  getResultsTemplate () {
    return this.state.results.map((val, i) => {
      const style = this.state.selectedIndex === i ? styles.selectedResultStyle : styles.resultsStyle;
      return <div key={i} style={style}>{val.title}</div>
    })
  }
  ...
  render () {
    ...
    return (
      <div className={mainClass} style={{width:width}}
...
FuzzySearch.propTypes = {
  ...
  getResultsTemplate: PropTypes.func,
